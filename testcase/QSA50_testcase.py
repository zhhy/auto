...
class QSA50Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(len(self.sd.devicelist)):
            ...
            if 'bundleId' in desire_caps:
                desire_caps['bundleId'] = 'com.powerinfo.iLiveQSHuajiao'  # com.powerinfo.iLiveQSA50
                # desire_caps['waitForQuiescence'] = 'false'
            else:
                desire_caps['appPackage'] = 'com.powerinfo.iLiveQSA50'
                desire_caps['appActivity'] = 'com.powerinfo.iLiveQSTZ.AudioOnlyLoginSettingActivity'
    ...
    def test_001参与者多次加入离开房间(self):
        ...
        for driver in self.driverlist:
            ...
            proc = pool.apply_async(a50_behavior.joinandleaveroom, (driver, self.roomid, self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_002参与者多次切后台(self):
        ...
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(a50_behavior.backapp, (driver, self.roomid, self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_004多次切换摄像头(self):
        ...
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(a50_behavior.switchcamera, (driver, self.roomid, self.device_name[index],))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_005参与者多次锁屏(self):
        ...
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(a50_behavior.lockscreen, (driver, self.roomid, self.device_name[index], ))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
    ...
    def test_006参与者切后台(self):
        ...
        a50_behavior.startnewroom(driver_zhubo, elementinfo)
    ...
    def test_007创建房间不加入返回(self):
        logger.info('test_004参与者多次创建房间')
        procs = []
        pool = multiprocessing.Pool(processes=len(self.driverlist))
        for driver in self.driverlist:
            index = self.driverlist.index(driver)
            proc = pool.apply_async(a50_behavior.startback, (driver, self.device_name[index],))
            procs.append(proc)
        for i in procs:
            i.get()
        for i in procs:
